...
import { summarizationPrompt as defaultSummarizationPrompt } from "../constants/prompt";
...
export class SummarizationService {
  ...
  private async summarizeWithOpenAI(
    ...
    const systemPrompt = options?.prompt || defaultSummarizationPrompt;
...
export const getSummarizationService = (
  ...
  if (!summarizationServiceInstance || openaiClient) {
    ...
    summarizationServiceInstance = new SummarizationService(openaiClient);
  }
  return summarizationServiceInstance;
};

// デフォルトのシングルトンインスタンス (従来の利用方法を維持する場合)
export const summarizationService = getSummarizationService();
